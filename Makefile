# This Makefile is for the Joe::Hacker extension to perl.
#
...
#     ABSTRACT => q[An easy way to authenticate users via LinkedIn.]
#     AUTHOR => [q[Asaf Klibansky <discobeta@gmail.com>]]
#     BUILD_REQUIRES => { ExtUtils::MakeMaker=>q[6.36] }
#     CONFIGURE_REQUIRES => {  }
#     DISTNAME => q[Joe-Hacker]
#     NAME => q[Joe::Hacker]
#     NO_META => q[1]
#     PREREQ_PM => { Carp=>q[0], Digest::MD5=>q[0], ExtUtils::MakeMaker=>q[6.36], JSON::Any=>q[0], LWP::UserAgent=>q[0], XML::Hash=>q[0] }
#     TEST_REQUIRES => {  }
#     VERSION => q[0.02]
#     VERSION_FROM => q[lib/Linkedin/OAuth2.pm]
#     dist => { PREOP=>q[$(PERL) -I. "-MModule::Install::Admin" -e "dist_preop(q($(DISTVNAME)))"] }
#     realclean => { FILES=>q[MYMETA.yml] }
#     test => { TESTS=>q[t/Linkedin-OAuth2.t] }
...
PERL_ARCHLIBDEP = /opt/local/lib/perl5/5.16.3/darwin-thread-multi-2level
LIBPERL_A = libperl.a
...
PERL_INCDEP = /opt/local/lib/perl5/5.16.3/darwin-thread-multi-2level/CORE
...
MAKEMAKER   = /opt/local/lib/perl5/site_perl/5.16.3/ExtUtils/MakeMaker.pm
MM_VERSION  = 7.1
MM_REVISION = 71000
...
PARENT_NAME = Joe
DLBASE = $(BASEEXT)
VERSION_FROM = lib/Linkedin/OAuth2.pm
OBJECT = 
LDFROM = $(OBJECT)
LINKTYPE = dynamic
BOOTDEP = 
...
H_FILES  = 
MAN1PODS = 
MAN3PODS = lib/Linkedin/OAuth2.pm

# Where is the Config information that we are using/depend on
CONFIGDEP = $(PERL_ARCHLIBDEP)$(DFSEP)Config.pm $(PERL_INCDEP)$(DFSEP)config.h

# Where to build things
INST_LIBDIR      = $(INST_LIB)/Joe
INST_ARCHLIBDIR  = $(INST_ARCHLIB)/Joe
...
TO_INST_PM = lib/Linkedin/OAuth2.pm

PM_TO_BLIB = lib/Linkedin/OAuth2.pm \
	blib/lib/Linkedin/OAuth2.pm
...
manifypods : pure_all  \
	lib/Linkedin/OAuth2.pm
	$(NOECHO) $(POD2MAN) --section=3 --perm_rw=$(PERM_RW) -u \
	  lib/Linkedin/OAuth2.pm $(INST_MAN3DIR)/Linkedin::OAuth2.$(MAN3EXT) 
...
TEST_FILE = test.pl
TEST_FILES = t/Linkedin-OAuth2.t
TESTDB_SW = -d
...
ppd :
	...
	$(NOECHO) $(ECHO) '    <ABSTRACT>An easy way to authenticate users via LinkedIn.</ABSTRACT>' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '    <AUTHOR>Asaf Klibansky &lt;discobeta@gmail.com&gt;</AUTHOR>' >> $(DISTNAME).ppd
	...
	$(NOECHO) $(ECHO) '        <REQUIRE NAME="XML::Hash" />' >> $(DISTNAME).ppd
...
pm_to_blib : $(FIRST_MAKEFILE) $(TO_INST_PM)
	$(NOECHO) $(ABSPERLRUN) -MExtUtils::Install -e 'pm_to_blib({@ARGV}, '\''$(INST_LIB)/auto'\'', q[$(PM_FILTER)], '\''$(PERM_DIR)'\'')' -- \
	  lib/Linkedin/OAuth2.pm blib/lib/Linkedin/OAuth2.pm 
...
installdeps ::
	$(PERL) Makefile.PL --config= --installdeps=XML::Hash,0

installdeps_notest ::
	$(PERL) Makefile.PL --config=notest,1 --installdeps=XML::Hash,0

upgradedeps ::
	$(PERL) Makefile.PL --config= --upgradedeps=XML::Hash,0,JSON::Any,0,Digest::MD5,0,LWP::UserAgent,0,Carp,0

upgradedeps_notest ::
	$(PERL) Makefile.PL --config=notest,1 --upgradedeps=XML::Hash,0,JSON::Any,0,Digest::MD5,0,LWP::UserAgent,0,Carp,0

listdeps ::
	@$(PERL) -le "print for @ARGV" XML::Hash

listalldeps ::
	@$(PERL) -le "print for @ARGV" XML::Hash JSON::Any Digest::MD5 LWP::UserAgent Carp
